...
function cnShi(sLength, sType) { //诗句，调用前限制输入的数量
    var shiResult = "",
        j,
        sName,
        halfLen = Math.round(sLength * 2 / 3),
        restLen = sLength - halfLen;
    if (sType === 8) { // 律诗
        $("#t_type").text("题");
        sName = ["首", "颔", "颈", "尾"];
        for (j = 0; j < 4; j += 1) {
            shiResult += "<li><span class=\"tip\">" + sName[j] + "</span>" + getChar(sLength) + "，" + getChar(sLength) + "。</li>";
        }
    } else if (sType === 4) { // 绝句
        $("#t_type").text("题");
        sName = ["起", "承", "转", "合"];
        for (j = 0; j < 2; j += 1) {
            shiResult += "<li><span class=\"tip\">" + sName[j * 2] + "</span>" + getChar(sLength) + "，</li><li><span class=\"tip\">" + sName[j * 2 + 1] + "</span>" + getChar(sLength) + "。</li>";
        }
    } else if (sType === 2) { // 对联
        $("#t_type").text("横");
        if (sLength > 8) {
            shiResult = "<li><span class=\"tip\">上</span>" + getChar(halfLen) + " " + getChar(restLen) + "</li><li><span class=\"tip\">下</span>" + getChar(halfLen) + " " + getChar(restLen) + "</li>";
        } else {
            shiResult = "<li><span class=\"tip\">上</span>" + getChar(sLength) + "</li><li><span class=\"tip\">下</span>" + getChar(sLength) + "</li>";
        }
    } else {
        $("#t_type").text("题");
        if (parseInt(sType, 10) < 21) {
            sName = ["壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾"];
            for (j = 0; j < parseInt(sType, 10) / 2; j += 1) {
                shiResult += "<li><span class=\"tip\">" + sName[j] + "</span>" + getChar(sLength) + "，" + getChar(sLength) + "。</li>";
            }
        } else {
            for (j = 0; j < parseInt(sType, 10) / 2; j += 1) {
                if (j < 9) {
                    shiResult += "<li><span class=\"tip\">0" + (j + 1) + "</span>" + getChar(sLength) + "，" + getChar(sLength) + "。</li>";
                } else {
                    shiResult += "<li><span class=\"tip\">" + (j + 1) + "</span>" + getChar(sLength) + "，" + getChar(sLength) + "。</li>";
                }
            }
        }
    }
...
$("#title").text(getLen.title());
$("#author").text("作者：" + getLen.author());
$("#content_ins").html(cnShi(15, 2));
$("#button0").on("click", function() {
    var dlLen = getRandomInt(5, 16);
    $("#title").text(getLen.hengpi());
    $("#author").text("出自：" + getLen.author());
    $("#content_ins").html(cnShi(dlLen, 2));
});
$("#button1").on("click", function() {
    $("#title").text(getLen.title());
    $("#author").text("作者：" + getLen.author());
    $("#content_ins").html(cnShi(5, 4));
});
$("#button2").on("click", function() {
    $("#title").text(getLen.title());
    $("#author").text("作者：" + getLen.author());
    $("#content_ins").html(cnShi(5, 8));
});
$("#button3").on("click", function() {
    $("#title").text(getLen.title());
    $("#author").text("作者：" + getLen.author());
    $("#content_ins").html(cnShi(7, 4));
});
$("#button4").on("click", function() {
    $("#title").text(getLen.title());
    $("#author").text("作者：" + getLen.author());
    $("#content_ins").html(cnShi(7, 8));
});
//});