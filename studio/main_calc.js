    ...
    $(document).ready(function() {
        ...
        $("#calc").on("input", function() {
            var newInput = $("#calc").val().split(" ").join(""),
                calcResult;
            if (newInput === "") {
                $("#calc_tip2").attr("class", "").text("");
                $("#calc_result").attr("class", "").text("归零");
            } else if (newInput.match(/[^0-9\+\-\*\/\.]+/g) !== null) {
                $("#calc_tip2").text("输入不正确").attr("class", "error");
                $("#calc_result").text("错误").attr("class", "error");
            } else if (newInput.match(/[\-\+\*\/]{2,}/g) !== null) {
                $("#calc_tip2").text("同时输入了多个运算符").attr("class", "warn");
                $("#calc_result").text("错误").attr("class", "error");
            } else if (newInput.match(/^[\+\*\/]+/g) !== null) {
                $("#calc_tip2").text("开头不要运算符号").attr("class", "warn");
                $("#calc_result").text("错误").attr("class", "error");
            } else if (newInput.match(/\D$/g) !== null) {
                $("#calc_tip2").text(" 请继续输入数字").attr("class", "warn");
            } else {
                $("#calc_tip2").text("计算结果").attr("class", "");
                calcResult = inCalc(newInput);
                $("#calc_result").text(calcResult).attr("class", "");
