...
function getSystemStr() {
  var iconDom = '';
  if ( ua_s[0] === 'android' ) {
    iconDom = "<img src=\"standalone/system/" + ua_s[0].replace('s-', '') + ".svg\" alt=\"" + ua_s[0] + "\">"
  } else {
    iconDom = "<svg><use xlink:href=\"symbol.svg#" + ua_s[0] + "\"></use></svg>"
  }
  return "<div class=\"system " + icon_s + "\">" + iconDom + "<div class=\"text\">" + ua_s[1] + "</div></div>"
}
...
if (/triden|msie|windows phone/i.test(ua)) {
  ...
  document.getElementById("icon").innerHTML = getBrowserStr() + getSystemStr() + "<div class=\"hardware " + icon_h + "\"><svg><use xlink:href=\"symbol.svg#" + ua_h[0] + "\"></use></svg><div>" + ua_h[1] + "</div></div>";
}
